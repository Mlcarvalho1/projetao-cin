...
void Sha256::update(const uint8_t* data, size_t len) {
    ...
    while (len > 0) {
        ...
        const size_t take  = (space < len) ? space : len;
...
void Sha256::finalize(uint8_t out[HASH_SIZE]) {
    ...
    for (int i = 7; i >= 0; --i) {
        buffer_[buffer_len_++] = uint8_t(bit_count_ >> (i * 8));
    }
    compress(buffer_);
...
void Sha256::hash(const uint8_t* data, size_t len, uint8_t out[HASH_SIZE]) {
    Sha256 h;
    h.update(data, len);
    h.finalize(out);
}
